...
update_game :: proc(game: ^Game, delta: f32) {
    update_player(&game.player, game, delta)
}
...
load_fonts :: proc(game: ^Game) {
    game.fonts[FontFace.Silver] = SDL_TTF.OpenFont(
        "assets/fonts/silver/Silver.ttf",
        font_size,
    )
    if game.fonts[FontFace.Silver] == nil {
        fmt.eprintln(
            "ERROR: Failed to load font Silver: %s",
            SDL.GetErrorString(),
        )
        os.exit(1)
    }
...
create_text_sprites :: proc(game: ^Game) {
    ...
    sub_title.destination.x =
        game.window_width / 2 - sub_title.destination.w / 2
